...
def MailSender(filename,time):
    try:
        fromaddr = "jayshenkar7@gmail.com"
        toaddr = "jay.shenkar@mitaoe.ac.in"
    ...
    #File sent (the attatchment) is open in read binary form (rb)
...
#encoding and decoding algo used here (encryption purpose)   
        ...
        s.login(fromaddr,"fzol gvwl otez nrvd")
...
def ProcessLog(log_dir = 'Marvellous'):
    ...
    separator = "-" * 80
    log_path = os.path.join(log_dir, "MarvellousLog%s.log" % (time.ctime().replace(' ', '_').replace(':', '-')))
    f = open(log_path, 'w')
    f.write(separator + "\n")
